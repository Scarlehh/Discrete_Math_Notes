...
\begin{document}
	...
	\begin{description}
		...
		\begin{enumerate}
			\item Contest - \underline{free grammars} where we can replace \underline{any} occurrence of $\langle T\rangle$ by $w$ if $\langle T\rangle \rightarrow w$ is one of our production rules.
		...
		\item[Definition:] A \underline{context free grammar} $(V, A, \langle s\rangle P)$ consists o a finite set $V$, a subset $A$ of $V$, an element $\langle s\rangle$ of $V \backslash A$, and a finite subset $P$ of the Cartesian product $V\backslash A \times V^*$.
	...
	\begin{description}
		...
		\item[Definition:] Lt $(V, A \langle s\rangle, P)$ be a phrase structure grammar. The language generated by this grammar is the subset $L$ or $A^*$ defined by $L \{w \in A^* \mid \langle s \rangle \overset{*}{\Rightarrow} w \}$
